...
int aee_is_enable(void)
{
	...
	if (node) {
		...
		pr_debug("%s: Can't find chosen node\n", __func__);
	}
...
static int mboot_params_check_header(struct mboot_params_buffer *buffer)
{
	if (!buffer || (buffer->sz_buffer != mboot_params_buffer->sz_buffer)
		...
		|| buffer->off_lk + ALIGN(buffer->sz_lk, 64)
			!= buffer->off_llk) {
		pr_debug("mboot_params: ilegal header.");
		return -1;
	} else
		return 0;
}
		...
		size_t buffer_size)
{
	...
	if (mboot_params_save_old(buffer, buffer_size))
		pr_debug("mboot_params: failed to creat old buffer\n");
	...
	if (!mbootlog_buf)
		pr_debug("mboot_params: mbootlog_buf(SYS_LAST_KMSG) invalid");
...
static int __init dt_get_mboot_params(struct mem_desc_t *data)
{
	...
	sram = (struct mem_desc_t *)of_get_property(np_chosen,
						    ...
						    NULL);
	if (sram) {
		pr_debug("mboot_params:[DT] 0x%x@0x%x, 0x%x(0x%x)\n",
		...
		struct mboot_params_memory_info *p_memory_info)
{
	...
	if (sram->offset > sram->size) {
		...
		if (magic1 == MEM_MAGIC1 && magic2 == MEM_MAGIC2) {
			...
			pr_debug("mboot_params: [DT] 0x%x@0x%x\n",
					mini_size, mini_addr);
...
#else
	pr_debug("mboot_params: CONFIG_OF not set\n")
#endif

	pr_debug("mboot_params: buffer start: 0x%lx, size: 0x%zx\n",
