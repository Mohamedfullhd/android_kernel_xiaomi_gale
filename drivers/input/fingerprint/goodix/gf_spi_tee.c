...
static int gf_hw_power_enable(struct gf_device *gf_dev)
{
	...
	if (regulator_is_enabled(gf_dev->vreg) > 0) {
		...
		if (rc < 0) {
			...
			pr_info("regulator_enable success\n");
		}
		msleep(15);
...
static int gf_hw_power_disable(struct gf_device *gf_dev)
{
	int rc = 0;
    int voltage, enabled;
	...
	if (regulator_is_enabled(gf_dev->vreg) <= 0) {
		pr_info("fp_vdd_vreg is already disabled!\n");
	} else {
        rc = regulator_disable(gf_dev->vreg);
		if (rc < 0)
			pr_err("regulator_disable fail\n");
 		else
			pr_info("regulator_disable success\n");
...
static void gf_irq_gpio_cfg(struct gf_device *gf_dev)
{
	...
	if (node) {
		...
		irq = of_get_named_gpio(node,"int-gpios",0);
		gf_dev->irq_num = gpio_to_irq(irq);
			...
			unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK && gf_dev) {
		blank = *(int *)evdata->data;
...
static irqreturn_t gf_irq(int irq, void *handle)
{
	...
	//pr_info("[XMFP]: %s gf_dev->wait_finger_down:%d  gf_dev->fb_black:%d\n", __func__,gf_dev->wait_finger_down,gf_dev->fb_black == 1);
        if ((gf_dev->wait_finger_down == true) && (gf_dev->fb_black == 1)) {
		pr_info("[XMFP]: %s zcr enter fingerdown & fb_black then schedule_work\n", __func__);
...
{
	pr_info("[XMFP]: zcr entry start %s \n", __func__);
	mdss_prim_panel_fb_unblank(FP_UNLOCK_REJECTION_TIMEOUT);
        pr_info("[XMFP]: zcr entry end %s \n", __func__);
}
...
static const struct file_operations proc_file_goodix_ops = {
	.owner = THIS_MODULE,
	.open = proc_open,
	.read = seq_read,
...
static int gf_probe(struct platform_device  *pdev)
{
	...
	gf_dev->vreg = devm_regulator_get(&gf_dev->spi->dev, "VFP");
	if (gf_dev->vreg == NULL) {
		gf_debug(ERR_LOG, "devm_regulator_get VFP get fail\n");
		goto err_readid;
	} else {
		gf_debug(INFO_LOG, "devm_regulator_get VFP get success\n");
	}

	status = regulator_set_voltage(gf_dev->vreg, 3000000, 3000000);
	if (status < 0) {
		gf_debug(ERR_LOG, "regulator_set_voltage fail\n");
	    goto err_readid;
	} else {
		gf_debug(INFO_LOG, "regulator_set_voltage success\n");
	}

	/*status = regulator_enable(gf_dev->vreg);
	if (status < 0) {
		gf_debug(ERR_LOG, "regulator_enable fail\n");
		goto err_regulator_enable;
	} else {
		gf_debug(INFO_LOG, "regulator_enable success\n");
	}*/

	status = regulator_get_voltage(gf_dev->vreg);
	gf_debug(INFO_LOG, "regulator_get_voltage success, voltage:%d\n", status);
	...
	if((rx_test[0]!= 0x05) || (rx_test[1] != 0x01) || (rx_test[2] != 0x02) || (rx_test[3]!=0x01))
	{
...
static int check_hwid(struct spi_device *spi)
{
	...
	do {
		...
		if((rx_test[0]!= 0x05) || (rx_test[1] != 0x05) || (rx_test[2] != 0x02) || (rx_test[3]!=0x01))
		{
