			...
			size_t size)
{
	...
	if (total_size >= client->threshold_size) {
		if (client->task) {
			...
			IONMSG("warn: client:%s(%s) memory exceed threshold:%llu,heap:%u, pid:%d\n",
			...
			size_t size)
{
	...
	if (heap->type == ION_HEAP_TYPE_MULTIMEDIA_SEC) {
		...
		if (total_size < 0) {
			IONMSG("heap_id:%u underflow!, total_now[%lld--%lld]\n",
			       heap->id, total_size,
				     ...
				     const char *name)
{
	...
	client->proc_root = proc_create_data(client->display_name,
					     ...
					     client);
	if (!client->proc_root)
		ion_err("Failed to create client procfs at /proc/ion/clients/%s\n",
			client->display_name);
#endif
				      ...
				      unsigned long arg))
{
	...
	ret = ion_init_sysfs();
	if (ret) {
		IONMSG("ion: failed to add sysfs attributes.\n");
		goto err_sysfs;
	}
